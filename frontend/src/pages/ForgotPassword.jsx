  ...
  const requestOtp = async (data) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setStep('otp');
        reset({ email: data.email, otp: '', password: '' });
        ...
        <div className="text-center mb-8">
          <div className="flex justify-center mb-3">
            <div className="p-2 bg-brand-500/10 rounded-xl text-brand-500">
              {step === 'email' ? <KeyIcon className="w-6 h-6" /> : <ShieldCheckIcon className="w-6 h-6" />}
            </div>
          </div>
        ...
        {step === 'email' ? (
          ...
          <form onSubmit={handleSubmit(resetPassword)} className="space-y-4">
            ...
            <button
              ...
              className="w-full text-xs font-semibold text-slate-400 hover:text-brand-500 transition-colors"
            >
